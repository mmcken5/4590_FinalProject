...
This is a R Markdown document for the VBA/Excel DCF Model we recreated in RStudio. Our group broke the model into three parts: *inputs*, *function*, and *calling*.

# Input List

To start, we included all the inputs needed for our company to start the valuation in the list **Input_List**. The list includes all the essential financial information of our company. Not only do we have the financial information of the company, but we also included the assumptions we need for the model.
...
For **dcf_function**, we call only one argument which is our input list. The function will automatically call the variable when needed in the model. Similiar to the Excel, we project to the excess years and terminal. Instead of printing into cells, R will create vectors of the new forecast values. 

At the end of **dcf_function**, the function will print out equity value, as well as equity value per share. 
...
So now we call **dcf_function** with **Input_List** and we should get the exact same value as the model from Excel/VBA. Voila!
